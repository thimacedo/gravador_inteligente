...
def extrair_palavras_gatilho(texto: str, palavras_gatilho: tuple) -> str | None:
...
def aplicar_cortes_audio(
    ...
    """
    cortes_unicos = sorted(set(cortes), key=lambda x: x[0])
        ...
        "operacoes": [
            {
                ...
                "similaridade_percent": (
                    round(op.similaridade * 100, 1) if op.similaridade else None
                ),
                "gatilho": op.gatilho,
            }
            for op in operacoes
...
def editar_boletim(
    ...
    config: EdicaoConfig | None = None,
    transcricao_previa: list[dict] | None = None,
    transcricao_json: str | Path | None = None,
) -> dict:
    """
...
def editar_boletins_diretorio(
    ...
    palavras_gatilho: tuple | None = None,
) -> list[dict]:
    """
    ...
    arquivos_audio = [f for f in entrada.iterdir() if f.is_file() and f.suffix.lower() in extensoes]
    ...
    # Resumo
    total_original = sum(
        r.get("duracao_original_s", 0) for r in resultados if r.get("status") == "sucesso"
    )
    total_final = sum(
        r.get("duracao_final_s", 0) for r in resultados if r.get("status") == "sucesso"
    )
...
def main_cli():
    ...
    parser.add_argument(
        "-w", "--janela", type=float, default=25.0, help="Janela de retrocesso em segundos"
    )
    parser.add_argument("--sem-log", action="store_true", help="Não gera log de exclusões")
        ...
        if resultado["status"] == "sucesso":
            ...
            if resultado.get("operacoes"):
                ...
                for op in resultado["operacoes"]:
                    ...
                    print(
                        f"  {tipo_str}: {op['inicio_s']}s → {op['fim_s']}s " f"({op['duracao_s']}s)"
                    )
