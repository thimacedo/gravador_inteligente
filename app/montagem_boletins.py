...
class AssetsConfig:
    ...
    encerramento: str = field(
        default=os.path.join(ASSETS_BOLETIM_DIR, "VHT_ENCERRAMENTO_BOLETIM.mp3")
    )
...
def separar_cabeca_off(
    audio: AudioSegment,
    pausa_ms: int | None = None,
    config: MontagemConfig | None = None,
    ...
    logger.info(f"Cabeça: {len(cabeca)/1000:.1f}s | OFF: {len(off)/1000:.1f}s")
...
def montar_boletim(
    ...
    config: MontagemConfig | None = None,
    pausa_override_ms: int | None = None,
) -> dict:
    """
...
def auditar_boletim(
    ...
    try:
        from app.tratamento_audio import measure_lufs
    ...
    try:
        from app.tratamento_audio import get_audio_info
