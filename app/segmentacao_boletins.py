...
def transcrever_audio(modelo, caminho_audio: str, idioma: str = DEFAULT_IDIOMA):
    ...
    result = modelo.transcribe(caminho_audio, language=idioma, fp16=False, verbose=False)
...
def construir_limites_boletins(
    ...
    set(marcadores_detectados.keys())
    ...
    if len(marcadores) < 3:  # Só temos B1 ou B1 + 1 detectado
        logger.warning(
            f"Apenas {len(marcadores)} boletins com marcadores detectados. Usando divisão igualitária."
        )
        ...
        conhecidos = sorted([m for m in marcadores if m["detectado"]], key=lambda x: x["inicio"])
        ...
        for num in range(2, numero_total_boletins + 1):
            ...
            if anterior is None and posterior is None:
                ...
                ultimo = conhecidos[-1]
                intervalo = (duracao_total - ultimo["inicio"]) / (
                    numero_total_boletins - ultimo["numero"] + 1
                )
...
def gerar_log_segmentacao(
    marcadores: list[dict], audio_path: str, duracao_total: float, saida_dir: str
) -> str:
...
class SegmentacaoConfig:
    ...
    duracao_total_override: float | None = None  # Usa em vez de transcrever


def segmentar_audio(
    caminho_audio: str | Path,
    config: SegmentacaoConfig | None = None,
    **kwargs,
) -> list[str]:
    """
...
def main_cli():
    ...
    parser.add_argument(
        "-o", "--output", default=None, help=f"Diretório de saída (padrão: {DEFAULT_OUTPUT_DIR})"
    )
    parser.add_argument(
        "-m",
        "--modelo",
    ...
    parser.add_argument(
        "--sem-transcricao", action="store_true", help="Pula transcrição e usa índices estimados"
    )
    ...
    try:
        ...
        print("=" * 60)
        exportar_boletins(audio, marcadores, output_dir)
