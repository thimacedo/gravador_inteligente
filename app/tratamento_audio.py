...
def corrigir_canal_morto(input_path: str | Path, output_path: str | Path) -> dict:
    ...
    logger.info(
        f"Canal morto detectado! Audio apenas no canal {channel} "
...
def normalize_volume(
    ...
    cmd_measure = [
        ...
        input_path,
        "-af",
        f"loudnorm=I={target_lufs}:TP={target_peak_db}:LRA=11:print_format=json",
        "-f",
        "null",
        "-",
    ]
    ...
    cmd_apply = ["ffmpeg", "-y", "-i", input_path, "-af", filter_str, "-ar", "44100", output_path]
...
def reduce_breath(
    ...
    10 ** (attenuation_db / 20)  # Converte dB para fator linear
...
def remove_silence(
    ...
    logger.info(
        f"  Removido {removed:.1f}s de silêncio ({original_duration:.1f}s → {new_duration:.1f}s)"
    )
...
def process(
    input_path: str | Path,
    output_path: str | Path,
    config: TratamentoConfig | None = None,
) -> dict:
    """
...
def main_cli():
    ...
    parser.add_argument(
        "--noise-strength", type=float, default=0.5, help="Força da redução de ruído (0-1)"
    )
